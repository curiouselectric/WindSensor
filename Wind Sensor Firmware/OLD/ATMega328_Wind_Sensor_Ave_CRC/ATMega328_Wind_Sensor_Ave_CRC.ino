...
// reserve  bytes for the data:

// String inputString = "";   // a String to hold incoming data
// String errorString = "";   // a String to hold incoming data
// String returnString = "";  // A string to hold the returned data

char inputString[25];    // A char array to hold incomming serial data
char errorString[15];    // A char array to hold error info
char returnString[100];  // A char array to hold returned serial data
char buff[10];           // A char array to hold converted values
  ...
  if (!t1S.isFirstIteration()) {
    ...
    if (DEBUG_DATA_1S == true) {
      ...
      for (int y = 0; y < 8; y++) {
        ...
        Serial.print(wind_vane_data.direction_array_values[y]);
        Serial.print(F("\t :"));
      }
      Serial.print(F(" N: "));
...
void tap(Button2& btn) {
  ...
  if (btn == buttonA) {
    if (checkData.vane_training_mode == true) {
      ...
      strcpy (returnString, "aaI");
      strcat (returnString, UNIT_ID);
      strcat (returnString, "WVOK=");
      strcat (returnString, wind_vane_data.vane_directions[vane_training_direction]);
      if (ADD_CRC_CHECK) {
  ...
  if (stringComplete == true) {
    ...
    if (checkData.error_flag == true) {
      ...
      if (checkData.baud_return_flag == true) {
        ...
        returnString += (String)UNIT_ID;
        returnString += "WVCONm";
        dtostrf(wind_speed_data.wind_speed_conv_m, 1, 4, buff);  //4 is mininum width, 6 is precision
        returnString += buff;
        returnString += "c";
