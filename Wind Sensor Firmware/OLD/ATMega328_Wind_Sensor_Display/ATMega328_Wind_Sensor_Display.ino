...
void sendData(int average_time)
{
  ...
  // also update the display with the max and min:
  display_wind_max_min();
  // also update the wind vane information:
  display_vane_data();
}
...
void display_wind_speed_data()
{
  // This routine displays the various parameters on the OLED screen
  // Clears just the area for more data (not whole screen).
  // Need to clear the tiles from 0,0 to 8,4
  u8x8.setFont(u8x8_font_px437wyse700b_2x2_r);
  u8x8.setCursor(0, 0);
  u8x8.drawString(0, 0, "    ");

  static char disp[5];
  if (wind_speed_data.data_1s < 10)
  {
    u8x8.setCursor(2, 0);
    dtostrf(wind_speed_data.data_1s, 3, 1, disp);
  }
  else if (wind_speed_data.data_1s < 100)
  {
    u8x8.setCursor(0, 0);
    dtostrf(wind_speed_data.data_1s, 3, 1, disp);
  }
  ...
  {
    u8x8.setCursor(0, 0);
    dtostrf(wind_speed_data.data_1s, 3, 0, disp);
  }
  u8x8.print(disp);
  // Show the instantatneous wind vane direction:
  u8x8.setFont(u8x8_font_amstrad_cpc_extended_f);
  u8x8.setCursor(6, 2);
  u8x8.print("  ");
  u8x8.setCursor(6, 2);
  u8x8.print(wind_vane_data.return_direction(analogRead(VANE_PIN))); 
}

void display_wind_max_min()
{
  u8x8.setFont(u8x8_font_amstrad_cpc_extended_f);
  u8x8.setCursor(12, 0);
  u8x8.print("    ");
  ...
  u8x8.print("    ");
  u8x8.setCursor(12, 0);
  u8x8.print((int)wind_speed_data.data_max);
  u8x8.setCursor(12, 2);
  u8x8.print((int)wind_speed_data.data_min);
}

void display_vane_data()
{
  // Show the wind vane data:
  // Want to show a small bar graph depedning upon the values in the vane array.
  // First lets find the max value
  float vane_max = 0;
  for (int y = 0; y < 8; y++)
  {
    if (wind_vane_data.direction_array_values[y] > vane_max)
    {
      vane_max = wind_vane_data.direction_array_values[y];
    }
  }
}
...
void display_background()
{
  ...
  u8x8.clear();
  u8x8.setFont(u8x8_font_amstrad_cpc_extended_f);
  u8x8.drawString(9, 0, "Mx");
  u8x8.drawString(9, 2, "Mn");
  u8x8.drawString(0, 7, "N   E   S   W");
}
...
void display_setup()
{
  ...
  u8x8.inverse();
  u8x8.setFont(u8x8_font_px437wyse700b_2x2_r);
  u8x8.drawString(4, 1, "WIND");
  u8x8.drawString(2, 3, "SENSOR");
  u8x8.drawString(4, 5, "UNIT");
  u8x8.setFont(u8x8_font_amstrad_cpc_extended_f);
  u8x8.drawString(0, 7, "curiouselectric");
  u8x8.noInverse();
}
